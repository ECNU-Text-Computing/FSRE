...
class CNNSentenceEncoder(nn.Module):
    ...
    # len(indexed_tokens), self.max_length - np.count_nonzero(pos1==0), self.max_length - np.count_nonzero(pos2==0)
...
class BERTSentenceEncoder(nn.Module):
    ...
    def __init__(self, pretrain_path, max_length, cat_entity_rep=False, mask_entity=False, backend_model=None): 
        ...
        elif backend_model == 'mtb':
            ckpt = torch.load("Your path to MTB")
            #import pdb
            #pdb.set_trace()
            temp = OrderedDict()
            ori_dict = self.bert.state_dict()
    ...
    def forward(self, inputs, cat=True):
        if self.soft_prompt:
            # print(inputs['word'].shape[0])
            inputs['word'] = torch.cat([torch.full((inputs['word'].shape[0],self.n_tokens), 50256, device=inputs['word'].device), inputs['word']], 1)
            inputs['mask'] = torch.cat([torch.full((inputs['mask'].shape[0],self.n_tokens), 1, device=inputs['mask'].device), inputs['mask']], 1)
    ...
    def tokenize_prompt(self, raw_tokens):
        # token -> index
        tokens = ['[CLS]']
